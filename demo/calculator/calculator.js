...
const exprPow=defer();
const opPow=rule` '^' [${exprPow}|${exprValu}]`.on(() => h2((a,b)=>a**b) );
exprPow.set(rule`${exprValu}[${opPow}]`);

const opMul=rule` '*' ${exprPow}`.on(() => h2((a,b)=>a*b) );
const opDiv=rule` '/' ${exprPow}`.on(() => h2((a,b)=>a/b) );
const exprMulDiv=rule`${exprPow}[{${opMul}|${opDiv}}]`;

const opAdd=rule` '+' ${exprMulDiv}`.on(() => h2((a,b)=>a+b) );
const opSub=rule` '-' ${exprMulDiv}`.on(() => h2((a,b)=>a-b) );
